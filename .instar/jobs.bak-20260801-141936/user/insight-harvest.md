...
tags:
  - cat:learning
  - evolution
gate: curl -sf http://localhost:4040/evolution/learnings?applied=false 2>/dev/null | python3 -c "import sys,json; d=json.load(sys.stdin); exit(0 if len(d.get('learnings',[])) > 0 else 1)"
toolAllowlist:
  - Read
---
Harvest and synthesize learnings: curl -s http://localhost:4040/evolution/learnings?applied=false
...
curl -s -X POST http://localhost:4040/evolution/proposals -H 'Content-Type: application/json' -d '{"title":"...","source":"insight-harvest from LRN-XXX","description":"...","type":"...","impact":"...","effort":"..."}'

Then mark the relevant learnings as applied:
curl -s -X PATCH http://localhost:4040/evolution/learnings/LRN-XXX/apply -H 'Content-Type: application/json' -d '{"appliedTo":"EVO-XXX"}'
